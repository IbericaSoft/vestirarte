...
class QuickSkin {
   ...
   * @var string
   */
  public $extensions_dir    = 'quickskin/qx';      /* Directory where all extensions are stored */
...
class QuickSkinParser {
  ...
  */
  ...
  function compile( $compiled_template_filename = '', $data='', $supp_templates='', $extensions_dir='' ) {
    ...
    /* Fix. PHP 5.3.*. Antonio Gamez agamez@dobleh.com */    	
    if(stristr($this->template, "<!-- INCLUDE")) {
    	if ( preg_match('/<!-- INCLUDE ([a-zA-Z0-9\-_\/.]+) -->/', $this->template) ){    	
	        preg_match_all('/<!-- INCLUDE ([a-zA-Z0-9\-_\/.]+) -->/', $this->template, $matches);
	        foreach ($matches as $key){
	        	foreach( $key as $subfile ){

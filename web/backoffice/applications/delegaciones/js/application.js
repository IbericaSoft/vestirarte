...
function onStartApplication(){
	...
	//autocomplete
	$j( "#_delegacion" ).autocomplete({
		minLength: 2,
    	source: list,
        select: function(event, ui) {
        	//$j( "#_id_nombre" ).val( ui.item.id );
        	$j( "#_delegacion" ).val( ui.item.label );
		}
	});
...
function doSearch(){
	blockScreeen();
	var execute = {"type":"data","asynchronous":false,"class":$F('class'),"do":"filter","sessionclass":$F('sessionclass'),"_delegacion":$F('_delegacion'),"_estado":$F('_estado')};
	queryJson(execute);
	return false;
}
...
function refreshList(pagination,json){
	...
	$j.each(json, function(i, item) {
		...
		$j('<td/>',{class:'colSort'}).append(item.delegacion+"&nbsp;").appendTo(row);
 ...
 */
function doLink(){
	var title = prompt('Nombre del acceso directo...', 'Delegaciones');
	if ( !title )
		return;
	var win = getWindowsHandle(WINDOWS_HANDLE);
	var img = WEB_PATH+'/applications/delegaciones/images/delegaciones.png';
	
	var filter,doAc;
	if ( PAGE_NAME == 'list' ){
		filter = '&_delegacion='+$F('_delegacion')+'&_estado='+$F('_estado');
		doAc   = 'filter';
	} else {
		filter = '&id='+$j('#id').val();
		doAc   = 'doEdit';	
...
function doAceptar(){
	...
	if ( $j('#delegacion').val().replace(/^\s*/, '')=="" ){
		error += "<li>Delegacion</li>";
		$j('#delegacion').addClass("error");
	}
...
*/
function doEliminar(){
	$j( "<div>Se va a intentar eliminar esta delegaci&oacute;n\n&iquest;Seguimos adelante?</div>" ).dialog({
	      modal: true,
	      buttons: {
	        SI: function() {

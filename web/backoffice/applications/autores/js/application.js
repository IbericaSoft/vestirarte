...
function onStartApplication(){
	...
	if ( $j("body").hasClass("list") ){
	    ...
	    var colModel = [
	        ...
	        {title:"Autor", dataIndx: "autor", width:'100',dataType:"string"},
...
function valida(){
	...
	if ( $j('#autor').val()=="" ) 
		error += "&#9888; El autor está vacío<br>";
	
	if ( $j('input[name=estado]:checked').val()==undefined )
		error += "&#9888; Es necesario indicar si el autor está online o no<br>"; 

	if ( error ){
		showAlert ("El autor no es válido porque tiene estos errores:<br>"+error);
		return false;
	}
	return true;
}
...
*/
function doDelete(){
	$j( "<div>Esta operación eliminará este autor si no tiene dependencias. ¿Seguimos adelante?</div>" ).dialog({
	      modal: true,
	      buttons: {

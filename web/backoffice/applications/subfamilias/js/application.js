...
function onStartApplication(){
	...
	if ( $j("body").hasClass("list") ){
	    ...
	    var dataModel = {
	    	...
	    	dataType: 'json',
	    	getUrl: function(){
	    		blockScreeen("Cargando subfamilias ...");
	    		var data = "class="+$j("#class").val()+"&do=listAll&channel=json&sessionclass="+$j('#sessionclass').val()+"&"
	    			+ "_categoria="+$j('#_categoria').val() + "&" + "_familia="+$j('#_familia').val() + "&" + "_subfamilia="+$j('#_subfamilia').val() ;
...
function valida(){
	...
	if ( $j('#id_familia').val()=="" ) 
		error += "&#9888; La categoría-familia está vacía<br>";
	...
	if ( error ){
		showAlert ("La subfamilia no es válida porque tiene estos errores:<br>"+error);
		return false;
	}
	return true;
}
...
*/
function doDelete(){
	$j( "<div>Esta operación eliminará esta subfamilia si no tiene dependencias. ¿Seguimos adelante?</div>" ).dialog({
	      modal: true,
	      buttons: {

...
function onStartApplication(){
	...
	if ( $j("body").hasClass("list") ){
	    ...
	    var dataModel = {
	    	...
	    	dataType: 'json',
	    	getUrl: function(){
	    		blockScreeen("Cargando categorías ...");
...
function valida(){
	...
	if ( error ){
		showAlert ("La categoría no es válida porque tiene estos errores:<br>"+error);
		return false;
	}
	return true;
}
...
*/
function doDelete(){
	$j( "<div>Esta operación eliminará esta categoría si no tiene dependencias. ¿Seguimos adelante?</div>" ).dialog({
	      modal: true,
	      buttons: {
